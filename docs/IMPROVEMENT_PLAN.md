...
- **2026-07-25 (jsdom pinned to `~27.3.0` — an ESM/CJS break that only appears in production)** — `/api/generate-topic-content` died on Vercel with `ERR_REQUIRE_ESM` while working perfectly on the dev machine. Cause: `jsdom@27.4.0` bumped `html-encoding-sniffer` `^4`→`^6`, and v6 depends on the ESM-only `@exodus/bytes` while itself being CommonJS. Node 22.12+ permits `require(esm)`, so a local Node 24 hides the fault entirely; Vercel's serverless module loader does not, and the route fails at **import time** — which `next build` cannot catch either, since nothing is wrong at compile time. Pinned to `~27.3.0` (27.4.0 is the exact boundary; 26.x and 27.0–27.3 are all on the CJS-safe `html-encoding-sniffer@4`). **The guard to run before ever moving jsdom is `npm ls @exodus/bytes`, which must print `(empty)`.** Three lessons worth keeping: a green local test run and a green build prove nothing about module resolution in the deploy target; "works locally, 500s in prod" on a fresh deploy should send you to the *runtime* logs before any theory about timeouts; and a transitive dependency going ESM-only is a breaking change for CJS consumers no matter how the semver reads.
- **2026-07-25 (an optional feature must not be able to take the app down)** — The jsdom break above had a blast radius far larger than it should have: `lib/ai/tools/web.js` statically imports jsdom, `grounding.js` statically imported web.js, the content pipeline imports grounding.js, and both the sync route AND the Inngest worker import the pipeline. So a broken dependency belonging to **web grounding — a feature switched OFF by default —** made `/api/generate-topic-content` and `/api/inngest` return 500 at import time. The `/api/inngest` casualty was the expensive one: `/enqueue` does not import the worker, so it kept returning `202` and writing `generation_jobs` rows while the serve endpoint was dead, leaving jobs at `status='queued'`, `started_at=null`, `error=null` — success-looking telemetry for work that could never run. `grounding.js` now imports web.js **dynamically inside `gatherGrounding`**, so jsdom loads only when grounding actually runs (`mermaid.js` already did this correctly with `await import('jsdom')`). Rule to carry forward: a heavy or fragile dependency behind an optional flag belongs behind a dynamic import, not a top-level one — otherwise "off" still means "can crash everything". Diagnostic that would have found it in one step: `curl -i https://<domain>/api/inngest`.
- **2026-07-25 (a JSON parse error is never the real error)** — `Unexpected token '<', "<!DOCTYPE "...` is what `res.json()` says when the platform returned an HTML error page instead of the handler's JSON, and it buried the actual cause twice in one session (first suspected as a timeout, actually the ERR_REQUIRE_ESM crash above). `lib/http/read-json.js` now reads the body as text first and maps the status to a real explanation; a regression test asserts the words "Unexpected token" can never reach a user. Wired into both learn pages' generation and artifact calls.
