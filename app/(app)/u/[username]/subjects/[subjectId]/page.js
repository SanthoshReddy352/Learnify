...
export default function PublicSubjectPage() {
  ...
  const loadPublicData = async () => {
    try {
      ...
      // 2. Fetch Topics via the sanitized shared_topics view.
      // PRIVACY: the view exposes only the shared material — never the
      // author's progress (status/SM-2) or private notes. The graph shows
      // every node in a neutral 'available' state for visitors.
      const { data: topicsData, error: topicsError } = await supabase
        .from('shared_topics')
        .select('id, title, description, estimated_minutes, difficulty, subject_id, created_at')
        .eq('subject_id', subjectId)
        .order('created_at', { ascending: true })

      if (topicsError) throw topicsError
      setTopics((topicsData || []).map(topic => ({ ...topic, status: 'available' })))
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="container mx-auto px-4 md:px-6 pt-4 shrink-0">
            <TabsList className="bg-foreground/5 border border-border/60 p-1 w-full sm:w-auto grid grid-cols-3 sm:flex">
          ...
          <TabsContent value="graph" className="flex-1 h-full p-0 data-[state=active]:flex flex-col overflow-hidden pb-[env(safe-area-inset-bottom)]">
           ...
           <TabsContent value="topics" className="flex-1 overflow-y-auto p-6 space-y-4 pb-[calc(1.5rem+env(safe-area-inset-bottom))] container mx-auto">
             {topics.map((topic) => (
               <Card key={topic.id} className="glass-card hover:bg-foreground/5 transition-all border-border/60">
                 <CardHeader>
