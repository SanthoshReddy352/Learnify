...
import GamificationPanel from '@/components/sub-components/GamificationPanel'
import WeeklyGoalPanel from '@/components/sub-components/WeeklyGoalPanel'
import GlobalReviewsWidget from '@/components/GlobalReviewsWidget'
import { getGlobalAnalytics, getStudyTimeByWeek, getAllDueReviews, getGamificationCounts, getEngagementData } from '@/lib/analytics'
...
export default function Dashboard() {
  ...
  const [gamStats, setGamStats] = useState({ topicsCompleted: 0, subjectsCompleted: 0, reviewsCompleted: 0, streakDays: 0 })
  // Raw study logs + goal target for the P11.2 panel; day/week bucketing happens
  // client-side so it lands in the learner's own timezone.
  const [engagement, setEngagement] = useState({ logs: [], weeklyGoal: DEFAULT_WEEKLY_GOAL })
  ...
  const loadAnalytics = async (userId) => {
    const [globalStats, weeklyStats, dueReviews, gamCounts, engagementData] = await Promise.all([
      getGlobalAnalytics(userId),
      getStudyTimeByWeek(userId),
      getAllDueReviews(userId),
      getGamificationCounts(userId),
    ...
    // Derived gamification stats (P7.5) — computed from existing progress, not
    // a stored counter, so there is nothing to cheat and no table to migrate.
    const subjectStats = globalStats.subjectStats || []
    setGamStats({
      topicsCompleted: subjectStats.reduce((sum, s) => sum + (s.masteredTopics || 0), 0),
      subjectsCompleted: subjectStats.filter((s) => s.totalTopics > 0 && s.masteredTopics >= s.totalTopics).length,
      reviewsCompleted: gamCounts?.reviewsCompleted || 0,
      streakDays: gamCounts?.streakDays || 0
    })
  }
  ...
  return (
    <>
        ...
        {subjects.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.map((subject) => (
              <Card
                key={subject.id}
                className="glass-card hover:bg-foreground/5 transition-all cursor-pointer group border-border/60 hover:border-primary/30"
              ...
              >
                <CardHeader>
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-[160px] bg-card/95 backdrop-blur-sm border-border">
                ...
                <CardContent>
                  <div className="flex items-center justify-between text-sm mt-4 pt-4 border-t border-border/60">
        ...
        {/* Progress & Achievements (P7.5) + this week's goal and streak (P11.2) */}
        {subjects.length > 0 && (
