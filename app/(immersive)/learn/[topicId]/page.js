...
// to route generation through the async background worker with live progress.
const ASYNC_GENERATION = process.env.NEXT_PUBLIC_ASYNC_GENERATION === 'true'
...
export default function LearnPage() {
  ...
  const handleRegenerateContent = async () => {
    ...
    try {
        if (ASYNC_GENERATION) {
            // Background worker + Realtime progress (P5.2). The worker writes the
            // content to `topics`, so re-read it once the job succeeds.
            await genJob.runTopicContent(body)
            ...
            // error page, and parsing that would report a JSON syntax error
            // instead of the timeout that actually happened.
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 selection:bg-primary/20 selection:text-primary overflow-hidden">
        ...
        <div className="max-w-2xl w-full relative z-10 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
          <div className="mb-8 flex justify-between items-end">
             <div>
                <Button variant="ghost" onClick={() => setShowFlashcards(false)} className="hover:bg-foreground/5 -ml-4 mb-2 md:mb-0">
          ...
          {/* Progress Bar */}
          <div className="w-full bg-foreground/5 h-1.5 rounded-full mb-8 overflow-hidden">
             <div 
          ...
          <div className="mt-8 md:mt-12 flex flex-col-reverse md:flex-row justify-between items-center gap-4">
            <Button 
                ...
                className="w-full md:w-auto glass border-border hover:bg-foreground/5 min-w-[120px]"
            >
              Previous
              <span className="ml-2 text-xs text-muted-foreground hidden md:inline-block border border-border px-1.5 rounded bg-black/20">â†</span>
            </Button>
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/20 selection:text-primary">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 pt-[env(safe-area-inset-top)] bg-background/95 backdrop-blur-md border-b border-border supports-[backdrop-filter]:bg-background/60">
          {/* Main Header */}
          <div className="container mx-auto px-4 md:px-6 py-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <Button variant="ghost" size="icon" onClick={() => router.push(`/subjects/${subject.id}`)} className="hover:bg-foreground/5 shrink-0">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                <div className="flex items-center gap-3 bg-foreground/5 px-3 py-2 rounded-full border border-border/60 shrink-0">
          ...
          {/* Progress Bar */}
          <div className="border-t border-border/60 bg-foreground/5">
      ...
      <div className="container mx-auto px-4 md:px-6 pt-[calc(120px+env(safe-area-inset-top))] md:pt-[calc(140px+env(safe-area-inset-top))] pb-[calc(3rem+env(safe-area-inset-bottom))] max-w-4xl">
        <Card className="glass-card mb-8">
          ...
          <CardContent className="prose dark:prose-invert max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-p:text-muted-foreground prose-p:leading-relaxed">
            {topic.description && (
              <div className="mb-6 md:mb-8 p-4 md:p-6 bg-foreground/5 rounded-xl border border-border/60">
            ...
            {topic.content && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
                  <h3 className="text-lg md:text-xl font-semibold text-foreground flex items-center gap-2 m-0">
                    <Sparkles className="h-5 w-5 text-primary" />
                    Comprehensive Guide
                  </h3>
                  <div className="flex items-center gap-2 flex-wrap">
                    <TtsControls text={topic.content} />
            ...
            {(!topic.content || topic.content === topic.description || topic.content.length < 50) && (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground bg-foreground/5 rounded-xl border border-border/60 border-dashed mt-8">
                {generating ? (
                    <GenerationProgress
                        indeterminate={!ASYNC_GENERATION}
            ...
            {/* Retrieval practice (P9.2) — self-testing beats re-reading */}
            {topic.content && topic.content.length >= 50 && subject?.id && (
              <div className="mt-8">
                <RetrievalPractice topicId={topic.id} subjectId={subject.id} />
              </div>
            )}

            {/* Interactive demo (P7.3) — sandboxed AI-generated widget */}
            {topic.content && topic.content.length >= 50 && (
              <div className="mt-8">
                {artifact ? (
                  <ArtifactFrame html={artifact.html} title={artifact.title} description={artifact.description} />
                ) : (
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 justify-center">
          <Button variant="outline" size="lg" onClick={handleShowFlashcards} className="w-full md:w-auto glass border-border hover:bg-foreground/5">
