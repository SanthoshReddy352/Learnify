...
// Relative, not aliased: ../mermaid-sanitize.js is itself dependency-free, so
// importing it keeps this module unit-testable.
import { repairMarkdownFences, mapOutsideFences } from '../mermaid-sanitize.js'
...
export function cleanTopicContent(rawContent) {
  ...
  // Close fences the model forgot to terminate, BEFORE any fence-aware pass —
  // an unterminated mermaid fence otherwise makes the whole rest of the lesson
  // look like diagram source to every step that follows.
  content = repairMarkdownFences(content)

  // `<br>` -> newline is a PROSE cleanup and must stay outside fenced blocks.
  // The diagram rules in this same file instruct the model to use `<br>` for
  // line breaks inside mermaid labels; rewriting those to literal newlines
  // produced exactly the construct mermaid cannot parse, so the cleanup was
  // breaking the diagrams the prompt had asked for.
  content = mapOutsideFences(content, (line) => line.replace(/<br\s*\/?>/gi, '\n'))
