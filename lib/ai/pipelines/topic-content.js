...
async function finalizeContent(rawContent, { userSecrets, onProgress }) {
...
async function runSectioned(inputs) {
  ...
  const sectionGrounding = groundingContext ? groundingContext.slice(0, 1500) : ''
  ...
  for (let i = 0; i < sections.length; i += 1) {
    // Section writing occupies the 15%–65% progress band.
    const pct = 15 + Math.round((i / sections.length) * 50)
    onProgress?.({
      progress: pct,
    ...
    const sectionText = await generateTextWithFallback({
      ...
      prompt: buildSectionPrompt({
        ...
        neighborContext,
        learnerContext,
        groundingContext: sectionGrounding
      }),
      maxOutputTokens: SECTION_MAX_TOKENS,
      userSecrets
    })

    parts.push(cleanTopicContent(sectionText))
  }
