...
// How many sections may be written at once.
//
// Sections are INDEPENDENT once the outline exists — each is prompted with the
// full outline for context but writes only its own heading — so writing them
// one at a time was pure wall-clock waste: an eight-section lesson paid for
// eight round-trips end to end and took minutes, with the learner watching a
// spinner the whole time.
//
// 4 rather than "all of them": the provider rate-limits, and a 429 halfway
// through costs more time than the extra parallelism saves.
...
async function runSectioned(inputs) {
  ...
  // Progress is now completion-counted rather than index-based: with parallel
  // writes, section 3 may well finish before section 2, and a bar driven by the
  // index of the section being started would jump around.
  let done = 0
  const parts = await mapWithConcurrency(sections, SECTION_CONCURRENCY, async (section, index) => {
    const text = await writeLessonSection({ ...inputs, sections, index })
    done += 1
    onProgress?.({
      progress: sectionProgress(done - 1, sections.length),
      stage: `Writing lesson — ${done}/${sections.length} sections done`
    })
    return text
  })

  // mapWithConcurrency preserves input order, so the lesson reads in outline
  // order no matter what order the calls returned in.
