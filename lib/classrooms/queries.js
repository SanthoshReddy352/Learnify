...
// Attention scoring + copy moved to the pure, unit-tested lib/teacher/insights.js
// in P12.2 (it had no tests here, and its wording is now held to the
// observational-language rule by an assertion).
const buildStudentAttentionMeta = describeStudentConcern
...
export async function getTeacherClassroomAnalytics(supabase, classroomId, teacherUserId) {
  ...
  // `concept_ledger` (P6.5) turns the heatmap's rows from topics into concepts,
  // but the column only exists after the P14 migration — so ask for it only
  // behind its flag, the same way lib/analytics.js getAllDueReviews does.
  const topicColumns = ['id', 'title', process.env.CONTENT_LEDGER === 'true' ? 'concept_ledger' : null]
    .filter(Boolean)
    .join(', ')
  ...
  const students = activeMembers
    .map((member) => {
      ...
      const idleDays = idleDaysFrom(lastActivity, now)
      ...
      return {
        ...
        overall,
        attention,
        // Per-student weekly timeline for the P12.3 detail view. Built from logs
        // already in memory, so it costs no extra query.
        weeklyTrend: buildWeeklyTrend({ logs: studentLogs, weeks: 6, now }),
        courses,
        weakTopics,
  ...
  // P12.1/P12.2/P12.4 — derived from the rows already fetched above, so the
  // richer page costs no extra database round trips.
  const heatmap = buildClassHeatmap({
    students: students.map((student) => ({ studentUserId: student.studentUserId, name: student.name })),
    topics,
    progressRows,
    reviewLogs: logs.filter((log) => log.session_type === 'review'),
    now
  })
  const trend = buildWeeklyTrend({ logs, weeks: 6, now })

  return {
    classroom: detail.classroom,
    heatmap,
    trend,
    headline: buildClassHeadline({
      summary: {
