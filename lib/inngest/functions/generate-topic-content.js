...
import { mapInWaves } from '@/lib/ai/concurrency'
...
// How many sections are written concurrently. Bounded rather than "all of
// them": the provider rate-limits, and a 429 partway through a lesson costs
// more wall clock than the parallelism saves. 3 also keeps the progress bar
// meaningful — it advances once per wave, so a 9-section lesson still reports
// three times rather than going quiet for the whole run.
const SECTION_WAVE_SIZE = 3
...
export const generateTopicContentJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      // ---- Step 4..N: write the sections, a wave at a time. -----------------
      //
      // Sections are INDEPENDENT once the outline exists — each is prompted with
      // the whole outline for context but writes only its own heading. Writing
      // them one after another meant an eight-section lesson cost eight serial
      // round-trips end to end, which is where the multi-minute wait came from.
      //
      // WHY WAVES AND NOT A WORKER POOL: Inngest re-executes this function body
      // on every invocation and matches work to prior results BY STEP ID, so
      // every step id has to be the same on every replay. A dynamic pool hands
      // out indices in completion order, which is not stable across replays. A
      // fixed wave is: section i is always in wave floor(i / SECTION_WAVE_SIZE)
      // and always has id `write-section-${i}`.
      //
      // Parallel steps are still separate invocations, so each one keeps its own
      // full time budget — the property that made the worker viable in the first
      // place is preserved.
      let assembled = ''
      if (sections.length > 0) {
        const parts = await mapInWaves(sections, SECTION_WAVE_SIZE, {
          // MUST be its own step. The body replays once per invocation, and an
          // unwrapped write here would re-run every replay and drag the progress
          // bar backwards all run.
          onWaveStart: ({ start, end, total }) =>
            step.run(`progress-wave-${start}`, () =>
              setJob({
                progress: sectionProgress(start, total),
                stage: end - start === 1
                  ? `Writing section ${end}/${total}`
                  : `Writing sections ${start + 1}-${end} of ${total}`
              })
            ),

          runItem: (_section, index) =>
            step.run(`write-section-${index}`, async () =>
              writeLessonSection({
                ...promptInputs,
                sections,
                index,
