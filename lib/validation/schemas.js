...
// --- Teacher-authored assessments -----------------------------------------

export const assessmentStatusSchema = z.enum(['draft', 'published', 'closed'])

// Settings a teacher can change on a paper. Every field is optional so the same
// schema serves both create and PATCH; the route supplies the defaults.
...
export const createAssessmentSchema = assessmentSettingsSchema.extend({
  classroomId: uuidSchema,
  subjectId: uuidSchema,
  title: z.string().trim().min(1).max(200),
})

// A question is EITHER a pinned bank item or a blueprint rule. The discriminated
// union is what stops a half-specified question reaching the database, where the
// same rule is repeated as a CHECK constraint — a blueprint missing its
// difficulty band would otherwise fail at exam-start time, in front of a class.
export const assessmentQuestionSchema = z.discriminatedUnion('source', [
  z.object({
    source: z.literal('item'),
    itemId: uuidSchema,
    points: z.number().min(0.5).max(100).optional().default(1),
    position: z.number().int().min(0).max(500).optional(),
  }),
  z.object({
    source: z.literal('blueprint'),
    conceptKey: z.string().trim().min(1).max(120),
    drawCount: z.number().int().min(1).max(20),
    difficultyMin: z.number().int().min(1).max(5).optional().default(1),
    difficultyMax: z.number().int().min(1).max(5).optional().default(5),
    points: z.number().min(0.5).max(100).optional().default(1),
    position: z.number().int().min(0).max(500).optional(),
  }),
]).refine(
  (q) => q.source !== 'blueprint' || q.difficultyMax >= q.difficultyMin,
  { message: 'difficultyMax must be greater than or equal to difficultyMin' }
)

// A question a teacher typed by hand. It becomes a normal assessment_items row,
// so it is indistinguishable from a generated one everywhere downstream.
export const manualAssessmentItemSchema = z.object({
  ...
  (item) => item.correctIndex < item.options.length,
  { message: 'correctIndex must point at one of the supplied options' }
)

export const assignAssessmentSchema = z.object({
  // An empty array means the WHOLE CLASS: assignments are stored as an
  // allow-list, and no rows is the "everyone, including people who join later"
  // case.
  memberIds: z.array(uuidSchema).max(500).optional().default([]),
})
...
// On-demand diagram repair (the lesson "Retry" button).
//
// `renderError` is the message from the CLIENT's failed mermaid.render(). It is
// the whole point of this request: the server only ever saw a parse() result,
// and render() failures carry strictly better repair information. It is model
// input only — never trusted, never executed, and bounded so a hostile client
// cannot use it to smuggle a large prompt.
export const repairDiagramRequestSchema = z.object({
  code: z.string().min(1).max(8000),
  renderError: z.string().max(1000).optional().default(''),
