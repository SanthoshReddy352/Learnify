...
/**
 * Submit a community resource contribution
 */
export async function submitCommunityResource(formData) {
  const supabase = await createClient()
  
  try {
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) throw new Error('You must be logged in to contribute')
    ...
    if (!drive_link || !subject || !name || !resource_type) {
    ...
    console.error('Error submitting community resource:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Fetch community resources by type
 */
export async function getCommunityResources(type) {
  const supabase = await createClient()
  
  try {
    // 1. Fetch Resources first
    ...
    const enrichedResources = resources.map(res => ({
      ...res,
      profiles: profileMap[res.user_id] || null
    }))

    return { success: true, resources: enrichedResources }
  } catch (error) {
    console.error(`Error fetching community resources (${type}):`, error)
    return { success: false, error: error.message }
  }
}
